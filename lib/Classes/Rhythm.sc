Rhythm {
  var <length, <size, <durations, <velocities;

  *new { |length, size, durations, velocities|
    ^super.new.init(length, size, durations, velocities);
  }

  init { |inLength, inSize, inDurations, inVelocities|
    length = inLength;
    size = inSize;

    // Scale durations to the rhythm's length and note count.
    durations = inDurations * length / size;

    // Normalize velocities to the input range.
    // TODO: Add scaling options to ctor.
    velocities = inVelocities / inVelocities.maxItem;
  }
}
